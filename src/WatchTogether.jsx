...
import { Star, Plus, Check, X, ChevronDown, ChevronUp, Trash2, Edit2, LogOut, Search } from ‘lucide-react’;
...
```
const toWatchRef = collection(db, 'couples', coupleId, 'toWatch');
const watchedRef = collection(db, 'couples', coupleId, 'watched');
...
try {
  if (isSignUp) {
    ...
    await setDoc(doc(db, 'users', userCred.user.uid), {
      email: email,
      name: userName || 'You',
      coupleId: coupleId || `couple_${Date.now()}`,
...
if (!selectedShow || !coupleId) return;
...
const itemRef = doc(collection(db, 'couples', coupleId, 'toWatch'));
...
const handleSubmitRating = async (yourRating, spouseRating, episodes = []) => {
if (!coupleId) return;
...
const watchedRef = doc(collection(db, 'couples', coupleId, 'watched'));
await setDoc(watchedRef, watchedItem);

await deleteDoc(doc(db, 'couples', coupleId, 'toWatch', itemToRate.id));
...
if (!coupleId) return;
const itemRef = doc(db, ‘couples’, coupleId, ‘toWatch’, itemId);
await setDoc(itemRef, { [field]: newPriority }, { merge: true });
};

const handleDeleteItem = async (itemId) => {
if (!coupleId) return;
await deleteDoc(doc(db, ‘couples’, coupleId, ‘toWatch’, itemId));
};
...
```
const showRef = doc(db, 'couples', coupleId, 'watched', showId);
    ...
    <div className="grid grid-cols-2 gap-4 mt-3 pt-3 border-t border-gray-700">
      <div>
        ...
        <StarRating 
          ...
          editable={editMode}
          onRate={(rating) => handleUpdatePriority(item.id, 'yourPriority', rating)}
        />
      </div>
      <div>
        ...
        <StarRating 
          ...
          editable={editMode}
          onRate={(rating) => handleUpdatePriority(item.id, 'spousePriority', rating)}
        />
      </div>
    </div>
...
const handleLoadEpisodesFromTMDB = async () => {
  ...
  try {
    const seasonData = await tmdbService.getSeasonDetails(itemToRate.tmdbId, 1); // Season 1
